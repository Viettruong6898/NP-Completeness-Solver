...
class Annealer(object):
    ...
    def save_state(self, fname=None):
        ...
        if not fname:
            ...
            fname = date + "_energy_" + str(self.all_energy()) + ".state"
    ...
    def anneal(self):
        ...
        while step < self.steps and not self.user_exit:
            ...
            wizard, num_violations = self.move()
            delta_violations = self.energy(wizard, num_violations)
            dE = (prevEnergy + delta_violations) - prevEnergy
            trials += 1
            if dE > 0.0 and math.exp(-dE / T) < random.random():
                ...
                if dE < 0.0:
                    improves += 1
                prevState = self.copy_state(self.state)
                prevEnergy = prevEnergy + delta_violations
                if prevEnergy < self.best_energy:
                    self.best_state = self.copy_state(self.state)
                    self.best_energy = prevEnergy
            if self.updates > 1:
